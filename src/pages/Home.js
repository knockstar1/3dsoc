...
export class Home {
  ...
  updateStarPhysics() {
    if (!this.stars || !this.starParticles) return;
    
    // Update diorama bounds for collision detection
    this.updateDioramaBounds();
    
    // Update main star particles
    ...
    for (let i = 0; i < this.starParticles.length; i++) {
      ...
      // Check collision with dioramas
      this.checkParticleCollision(particle);
      
      // Update geometry positions
      const index = i * 3;
      starsPositions[index] = particle.position.x;
      starsPositions[index + 1] = particle.position.y;
      starsPositions[index + 2] = particle.position.z;
    }
    this.stars.geometry.attributes.position.needsUpdate = true;
    
    // Update colored star particles
    if (this.coloredStars && this.coloredStarParticles) {
      this.coloredStars.forEach((coloredStars, starIndex) => {
        const positions = coloredStars.geometry.attributes.position.array;
        
        for (let i = 0; i < this.coloredStarParticles.length; i++) {
          const particle = this.coloredStarParticles[i];
        ...
        coloredStars.geometry.attributes.position.needsUpdate = true;
  ...
  updateDioramaBounds() {
    ...
    this.visibleDioramas.forEach(diorama => {
      if (!diorama.mesh.visible) return;
      
      // Create bounding box for each diorama
      const box = new THREE.Box3();
      box.setFromObject(diorama.mesh);
      
      // Expand the box slightly for better collision detection
      box.expandByScalar(1.0);
      
      this.dioramaBounds.push(box);
    });
  }

  checkParticleCollision(particle) {
    for (const bounds of this.dioramaBounds) {
      if (bounds.containsPoint(particle.position)) {
        // Particle is inside diorama bounds - deflect it
        const center = bounds.getCenter(new THREE.Vector3());
        const direction = particle.position.clone().sub(center).normalize();
        
        // Move particle to surface of bounding box
    ...
    const worldBounds = 75;
    if (Math.abs(particle.position.x) > worldBounds) {
      particle.velocity.x *= -1;
      particle.position.x = Math.sign(particle.position.x) * worldBounds;
    }
    if (Math.abs(particle.position.y) > worldBounds) {
      particle.velocity.y *= -1;
      particle.position.y = Math.sign(particle.position.y) * worldBounds;
    }
    if (Math.abs(particle.position.z) > worldBounds) {
      particle.velocity.z *= -1;
      particle.position.z = Math.sign(particle.position.z) * worldBounds;
    }
  }
  ...
  setupBackground() {
    // Create original style starfield with physics-based collision detection
    ...
    // Create floating particles that respect diorama boundaries
    this.starParticles = [];
    const starsVertices = [];
    ...
    for (let i = 0; i < 300; i++) {
      ...
      this.coloredStarParticles.push({
        position: new THREE.Vector3(x, y, z),
        velocity: new THREE.Vector3(
    ...
    // Create colored star systems with original colors
    ...
    // Create bounding boxes for dioramas for collision detection
    this.dioramaBounds = [];
  }
